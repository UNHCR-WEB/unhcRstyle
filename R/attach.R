## Copy of attach.R from tidyverse
## Copyright to the tidyverse team
...
msg <- function(..., startup = FALSE) {
  if (startup) {
    if (!isTRUE(getOption("unhcRstyle.quiet"))) {
...
unhcrstyle_attach <- function() {
  ...
  msg(
    cli::rule(
      ...
      right = paste0("unhcRstyle ", package_version("unhcRstyle"))
    ),
    startup = TRUE
  )
